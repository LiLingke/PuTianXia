...
public class StoreActivity extends Activity {
	...
	// 下拉框对象
	private SpinerPopWindow<String> mySpinerPopWindow1, mySpinerPopWindow2,
			mySpinerPopWindow3, mySpinerPopWindow4, mySpinerPopWindow5,
			mySpinerPopWindow6, mySpinerPopWindow7, mySpinerPopWindow8,
			mySpinerPopWindow9, mySpinerPopWindow10;
	// 适配下拉框内容的list
	private List<String> list1, list2, list3, list4, list5, list6, list7,
			list8, list9, list10;

	// 包含下拉框的TextView
	private TextView pyarea2, pyactualarea2, pyrent2, pystaltype2,
			pycondition2, pyfloor2, pyfloor3, pybusiness2, pyaddress2,
			pyaddress3;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		getWindow().setSoftInputMode(
         ...
         .discCacheFileNameGenerator(new Md5FileNameGenerator())//使用MD5对UIL进行加密命名
 ...
 ImageLoader.getInstance().init(config);
	...
	private OnClickListener clickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.pyarea2:
				...
				mySpinerPopWindow1
						.setSelectedCallback(new SelectedCallback<String>() {
							@Override
	...
	class MyGridAdapter extends BaseAdapter {
		...
		@Override
		public View getView(final int position, View contextView,
				ViewGroup parent) {
			ViewHolder holder = null;
			if (contextView == null) {
				holder = new ViewHolder();
				contextView = layoutInflater.inflate(R.layout.childgrid_item,
						null);
				holder.image = (ImageView) contextView
						.findViewById(R.id.child_iv);
				holder.btn = (Button) contextView
			...
			if (position == allSelectedPicture.size()) {
				...
				ImageLoader.getInstance().displayImage(
						"file://" + allSelectedPicture.get(position),
						holder.image);
